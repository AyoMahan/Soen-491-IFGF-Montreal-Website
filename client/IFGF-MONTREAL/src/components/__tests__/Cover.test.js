...
describe('Cover.vue', () => {
  it('renders the correct title and subtitle', () => {
    ...
    expect(wrapper.text()).toContain('IFGF MONTREAL');
    expect(wrapper.text()).toContain('In Christ Alone');
  });

  it('applies the correct background image', () => {
    const wrapper = mount(Cover);
    expect(wrapper.attributes('style')).toContain("background-image: url(/src/assets/christmas.jpg)");
  });
});
